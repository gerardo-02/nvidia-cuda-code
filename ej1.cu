...
__global__ void kernelHistograma(unsigned char *imagen, unsigned long size, unsigned int* histo){
        ...
        int var = IMDEP / blockDim.x;
        for (int i=0; i<=var; i++ ){
                if ( (threadIdx.x + (blockDim.x * i) ) < IMDEP){
                        temp[threadIdx.x + (blockDim.x * i)] = 0;
                }
        }
        ...
        for (int i=0; i<=var; i++){
                if ((threadIdx.x + (blockDim.x * i)) < IMDEP){
                        atomicAdd( &(histo[threadIdx.x + (blockDim.x * i) ]), temp[threadIdx.x + (blockDim.x * i) ] );
                }
        }
}
...
int main(void){
        ...
        double CPUMS = cpu_end - cpu_start;
        ...
        printf("Ganancia del cálculo en GPU respecto a GPU: %lf\n", CPUMS / (aveGPUMS / (numRuns*1000.0)));
